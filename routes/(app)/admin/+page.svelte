								...
								<td class="px-3 py-2 min-w-[7rem] w-28">
									<button
										class="flex items-center gap-2 text-xs px-3 py-0.5 rounded-lg 
											...
											{user.role === 'expert' && 'text-red-600 dark:text-red-200 bg-red-200/30'} 
											{user.role === 'Marketing' && 'text-yellow-600 dark:text-yellow-200 bg-yellow-200/30'} 
											{user.role === 'RH' && 'text-orange-600 dark:text-orange-200 bg-orange-200/30'}"
										on:click={() => {
											if (user.role === 'user') {
												...
												updateRoleHandler(user.id, 'RH');
											} else if (user.role === 'Marketing') {
												updateRoleHandler(user.id, 'expert');
											} else if (user.role === 'RH') {
												updateRoleHandler(user.id, 'pending');
											} else {
												updateRoleHandler(user.id, 'Marketing');
											}
										}}
									>
										<div
											class="w-1 h-1 rounded-full 
												...
												{user.role === 'expert' && 'bg-red-600 dark:bg-red-300'} 
												{user.role === 'Marketing' && 'bg-yellow-600 dark:bg-yellow-300'} 
												{user.role === 'RH' && 'bg-orange-600 dark:bg-orange-300'}"
										></div>
										{$i18n.t(user.role)}
									</button>
								</td>
