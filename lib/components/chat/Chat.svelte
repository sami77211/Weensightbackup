<script lang="ts">
  ...
  visible: boolean;
    ...
    themee = event.detail.themee; // Extract the theme value from the event detail
    console.log('Received Name from Child:', themee); // Check the received value
  }
			...
			selectedModelIds.map(async (modelId, _modelIdx) => {
				...
				if (model) {
					...
					if (model?.owned_by === 'openai') {
						...
						_response = await sendPromptOllama(model, prompt, responseMessageId, _chatId,themee);
					}
	...
	const sendPromptOllama = async (model, userPrompt, responseMessageId, _chatId,) => {
		let _response: string | null = null;themee
