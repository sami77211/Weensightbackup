...
const app = express();
const port = 3005; // Use a different port if 3000 is in use
// Enable CORS
app.use(cors());
...
// Route pour afficher les données de la collection themes_collection
...
app.put('/updateJobByReference', async (req, res) => {
  const { reference, post_name, mandatory_skills, optional_skils } = req.body;
    ...
    const result = await Job_sheetCollection.updateOne(
      { reference: reference }, // Cherche par la référence
      {
        $set: {
          ...
          optional_skils: optional_skils // Correction orthographique ici
        }
      }
    );
