				...
				reader.onload = async (e) => {
					...
					for (const [idx, row] of rows.entries()) {
						...
						if (idx > 0) {
								if (
									columns.length === 4 &&
								['admin', 'user', 'expert', 'Marketing','RH'].includes(columns[3].toLowerCase())
								) {
									const res = await addUser(
									localStorage.token,
									columns[0],
								...
								console.log(columns[3].toLowerCase());
						...
						{#if tab === ''}
							<div class="flex flex-col w-full">
								...
								>
									...
									<!-- Ajoutez ici le nouveau rôle -->
									<option value="Marketing"> {$i18n.t('Marketing')} </option> 
									<option value="RH"> {$i18n.t('RH')} </option> 
									<option value="expert"> {$i18n.t('expert')} </option>
								</select>
