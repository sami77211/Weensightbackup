...
>
	...
	>
		...
		{#if $user?.role === 'admin'||$user?.role ==='user'||$user?.role ==='RH'|| $user?.role ==='Marketing'||$user?.role ==='expert'}
		<div class="px-2.5 flex justify-center text-white dark:text-white">
			<a
				class="flex-grow flex space-x-3 rounded-xl px-2.5 py-2 hover:bg-gray-700 dark:hover:bg-gray-900 transition"
				href="/workspace"
		...
		<div class="relative flex flex-col flex-1 overflow-y-auto">
			...
			<div class="px-2 mt-0.5 mb-2 flex justify-center space-x-2">
				<div class="flex w-full rounded-xl" id="chat-search">
					...
					<input
					class="w-full rounded-r-xl py-1.5 pl-2.5 pr-4 text-sm bg-transparent dark:text-gray-300 outline-none recherche"
		...
		<div class="px-2.5">
			...
			<div class="flex flex-col font-primary">
				{#if $user !== undefined}
					...
					>
						<button
							class=" flex rounded-xl py-3 px-3.5 w-full hover:bg-gray-700 dark:hover:bg-gray-900 transition"
							on:click={() => {
...
<style>
  ...
  color: white; /* Définit la couleur du texte du placeholder en blanc */
  opacity: 1; /* Assure que la couleur soit complètement visible */
}
