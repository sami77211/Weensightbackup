		...
		>
			...
			{#if role === 'admin'|| role === 'expert'}
				...
				>
					...
					{/if}
					{#if $user.role==='expert'}
					<div class=" self-center font-medium">{$i18n.t('Expert Panel')}</div>
					{/if}
			...
			on:click={() => {
				...
				// Vide tout le stockage local si nécessaire (correctement appelé)
				localStorage.clear();
				// Redirige vers la page de connexion ou d'authentification
